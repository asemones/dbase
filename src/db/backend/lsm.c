...
int write_record_txn(storage_engine* engine ,db_unit key, db_unit value, int id){
    ...
    insert_list_txn(table->skip, key, value, id);
    table->num_pair++;
...
char * disk_read(storage_engine * engine, const char * keyword){ 
    /*search most recent ones first. since only one copy exists at each level, and l will yoink all copies,
    the assumption of newer verisions being at higher levels is true*/
    char * l_0_test;
    if ((l_0_test = scan_l_0(engine->meta->sst_files[0]->arr,keyword, engine->meta->sst_files[0]->len ))){
        return l_0_test;
    }
