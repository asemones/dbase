...
void seek(aseDB_iter * iter , const char * prefix){
    ...
    for(int i = 0; i < 6; i++){
        ...
        if (strncmp(m.key->entry, prefix, strlen(prefix))) continue;
        m.src = lvl_1_7;
        m.index = i;
        enqueue(iter->pq,&m);
     ...
     for (int i = 0; i < iter->l_0_sst_iters->len; i++){
        ...
        if (strncmp(merge.key->entry, prefix, strlen(prefix))) continue;
        merge.src = lvl_0;
        merge.index = i;
    ...
    for(int i =0; i < 2; i++){
        ...
        if (strncmp(entry.key->entry, prefix, strlen(prefix))) continue; /*should be greater than*/
